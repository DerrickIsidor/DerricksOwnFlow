...
> **Adapted for Claude Code.** This skill was authored by OpenAI for their Codex CLI (source: [openai/skills](https://github.com/openai/skills), Apache-2.0 — see `LICENSE.txt`). The doc-fetching workflow below works as-is via the `openaiDeveloperDocs` MCP server registered in this repo's `.mcp.json`. The "Codex self-knowledge" section further down describes OpenAI's Codex CLI product itself (`.codex/config.toml`, Codex surfaces, etc.) — it does not apply to Claude Code and can be ignored here; it's left intact for fidelity to the source.
...
The `openaiDeveloperDocs` MCP server is already registered for this project in `.mcp.json`. If MCP tools still fail or no OpenAI docs resources are available:

1. Run the install command yourself: `claude mcp add --transport http openaiDeveloperDocs https://developers.openai.com/mcp`
